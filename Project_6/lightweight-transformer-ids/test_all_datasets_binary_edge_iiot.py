"""Test script to evaluate trained Edge-IIoT binary model on all test datasets.

This script:
1. Loads the trained Edge-IIoT binary classification model
...
5. Saves results with timestamps in the same format as CIC-IoT results
"""
...
from pipeline.edge_iiot_pipeline import EdgeIIoTPreprocessor
...
def preprocess_dataset(df: pd.DataFrame, config: object, logger) -> Tuple[np.ndarray, np.ndarray, int, int]:
    """Apply the same preprocessing pipeline as in main_edge.py for Edge-IIoT dataset.
    ...
    # Binary encoding: 'normal' = 0, 'attack' = 1
    logger.info("Binary encoding: 'normal' = 0, 'attack' = 1")
    df[label_column] = df[label_column].map({'normal': 0, 'attack': 1})
    
    # Check for unmapped values
    if df[label_column].isna().any():
        logger.warning(f"Found {df[label_column].isna().sum()} unmapped label values!")
        df = df.dropna(subset=[label_column])
        logger.info(f"Dropped unmapped labels. New shape: {df.shape}")
    
    df[label_column] = df[label_column].astype(int)
    
    logger.info(f"Binary label distribution:")
    logger.info(f"  Class 0 (Normal): {(df[label_column] == 0).sum()} ({(df[label_column] == 0).sum()/len(df)*100:.2f}%)")
    logger.info(f"  Class 1 (Attack): {(df[label_column] == 1).sum()} ({(df[label_column] == 1).sum()/len(df)*100:.2f}%)")
    
    # Edge-IIoT specific config
    edge_config = {
        'label_column': 'label',
        'correlation_threshold': 0.9,
        'variance_threshold': 0.0,
        'drop_columns': [],
        'log_transform_columns': []  # Data is already normalized
    }
    
    # Create preprocessing pipeline
    logger.info("Applying Edge-IIoT preprocessing pipeline...")
    preprocessor = EdgeIIoTPreprocessor(df, config=edge_config)
    
    # Get labels before any dropping
    y_after_encoding = df[label_column].copy()
    
    # Apply preprocessing steps (same order as main_edge.py)
    logger.info("  Step 1/4: Dropping low variance features...")
    preprocessor.drop_low_variance_features()
    
    logger.info("  Step 2/4: Dropping specified columns...")
    preprocessor.drop_columns()
    
    logger.info("  Step 3/4: Dropping highly correlated features...")
    preprocessor.drop_highly_correlated_features()
    
    logger.info("  Step 4/4: Normalizing features...")
    # Check if data is already normalized
    df_temp = preprocessor.get_processed_data()
    feature_cols = [col for col in df_temp.columns if col != label_column]
    if len(feature_cols) > 0:
        sample_mean = df_temp[feature_cols].mean().mean()
        sample_std = df_temp[feature_cols].std().mean()
        if abs(sample_mean) < 0.1 and abs(sample_std - 1.0) < 0.5:
            logger.info("    Data appears already normalized, skipping normalization step")
        else:
    ...
    if label_column in df_processed.columns:
        ...
        # Label was dropped (zero variance), use encoded labels
...
def evaluate_dataset(
    ...
    evaluator = Evaluator(
        ...
        device=device,
        task_type='binary',  # Binary classification for Edge-IIoT
    )
...
<html>
<head>
    <title>Edge-IIoT Binary Classification Results - {timestamp}</title>
    <style>
        body {{
...
<body>
    <h1>Edge-IIoT Binary Classification - Multi-Dataset Evaluation Results</h1>
    <p style="text-align: center; color: #666;">Generated: {timestamp}</p>
    <p style="text-align: center; color: #666;">Binary Encoding: normal = 0, attack = 1</p>
"""
...
def main():
    ...
    config_profile = "default"  # Use default config profile (Edge-IIoT compatible)
    model_path = script_dir / "models/edgeiiot_binary.pth"
    data_dir = script_dir / "data/raw/edge_iiot"
    results_base_dir = script_dir / "results/edge_binary_classification/testing"
    
    # Check if model exists
    if not model_path.exists():
        print(f"ERROR: Model file not found: {model_path}")
        print("Please ensure the trained model is at the correct location.")
        sys.exit(1)
    ...
    logger.info("="*70)
    logger.info("EDGE-IIOT BINARY CLASSIFICATION - MULTI-DATASET EVALUATION")
    logger.info("="*70)
    ...
    test_datasets = [d for d in all_datasets if "train" not in d.name.lower()]
    ...
    try:
        ...
        # Get depth (number of layers) from checkpoint
        depth = 0
        for key in state_dict.keys():
            if "encoder.layers." in key:
                layer_num = int(key.split("encoder.layers.")[1].split(".")[0])
                depth = max(depth, layer_num + 1)
        
        logger.info(f"Model architecture from checkpoint:")
        logger.info(f"  Sequence length (num_features): {num_features}")
        logger.info(f"  Number of classes: {num_classes}")
        logger.info(f"  Depth (layers): {depth}")
        
        logger.info(f"Creating model with {num_features} features, {num_classes} classes, and {depth} layers")
        ...
        model = ModelFactory.create_model(
            ...
            model_config={
                ...
                "depth": depth,  # Use depth from checkpoint
